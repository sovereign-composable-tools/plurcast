...
async fn create_test_feed(
    ...
    for i in 1..=num_posts {
        ...
        let mut message = SSBMessage::new_post(&keypair.id, i as u64, previous.clone(), &content);
...
async fn test_import_from_empty_feed() {
    ...
    assert!(
        ...
        "Import should succeed with empty feed"
    );
...
async fn test_import_posts_from_feed() {
    ...
    let posts = sqlx::query!("SELECT id, content, status FROM posts ORDER BY created_at")
        .fetch_all(db.pool())
        .await
        .unwrap();
    ...
    for (i, post) in posts.iter().enumerate() {
        ...
        assert!(post
            .content
            .contains(&format!("Test post number {}", i + 1)));
    }
...
async fn test_import_skips_duplicates() {
    ...
    assert_eq!(
        posts_count.count, 2,
        "Should have 2 posts after first import"
    );
    ...
    assert_eq!(
        posts_count.count, 2,
        "Should still have 2 posts (no duplicates)"
    );
}
...
async fn test_import_skips_non_post_messages() {
    ...
    let mut post_message = SSBMessage::new_post(&keypair.id, 1, None, "This is a post");
    ...
    assert_eq!(
        posts.count, 1,
        "Should import only 1 post (skip 'about' message)"
    );
}
...
async fn test_import_handles_missing_feed() {
    ...
    assert!(
        result.is_err(),
        "Import should fail when feed doesn't exist"
    );
