...
pub async fn import_ssb(config: &Config, db: &Database, account: &str) -> Result<()> {
    ...
    let credentials =
        CredentialManager::new(cred_config).context("Failed to initialize credential manager")?;
    ...
    let messages = query_feed_messages(&feed_path).context("Failed to query SSB feed")?;
    ...
    debug!(
        "Found {} existing SSB message(s) in database",
        existing_ids.len()
    );
...
fn query_feed_messages(feed_path: &PathBuf) -> Result<Vec<SSBMessage>> {
    ...
    let entries = std::fs::read_dir(&messages_dir).context("Failed to read messages directory")?;
...
async fn get_existing_ssb_message_ids(db: &Database) -> Result<HashSet<String>> {
    ...
    let posts = db
        .query_posts_with_records(Some("ssb"), None, None, None, 10000)
        .await
