...
use libplurcast::platforms::{nostr::NostrPlatform, Platform};
...
async fn test_error_messages_dont_leak_credentials() {
    ...
    if let Err(e) = result {
        ...
        assert!(
            !error_msg.contains("nsec1secret"),
            "Error message should not contain key prefix"
        );
    }
...
fn test_validation_errors_dont_leak_content() {
    ...
    let validation_error = PlatformError::Validation(format!(
        "Content too large: {} bytes (max: 100000 bytes)",
        long_content.len()
    ));
...
fn test_error_context_includes_platform_not_credentials() {
    ...
    let errors = vec![
        PlatformError::Authentication(
            "Nostr authentication failed: Invalid key format".to_string(),
        ),
