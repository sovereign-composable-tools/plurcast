...
impl AccountManager {
    ...
    pub fn validate_account_name(name: &str) -> Result<()> {
        ...
        if name.is_empty() {
            return Err(
                AccountError::InvalidName("Account name cannot be empty".to_string()).into(),
            );
        }
        ...
        {
            return Err(AccountError::InvalidName(format!(
                "Invalid account name '{}'. Must be alphanumeric with hyphens/underscores only",
                name
            ))
            .into());
        }
    ...
    pub fn set_active_account(&self, platform: &str, account: &str) -> Result<()> {
        ...
        // Update state
        {
            ...
            state
                .active
                .insert(platform.to_string(), account.to_string());
        }
        ...
        // Update state
        {
            ...
            if let Some(active) = state.active.get(platform) {
                if active == account {
                    state
                        .active
                        .insert(platform.to_string(), "default".to_string());
                }
            }
        }
    ...
    fn save(&self) -> Result<()> {
        ...
        if let Some(parent) = self.state_file.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                AccountError::StateFile(format!("Failed to create directory: {}", e))
            })?;
        }
        ...
        #[cfg(unix)]
        {
            ...
            std::fs::set_permissions(&self.state_file, permissions).map_err(|e| {
                AccountError::StateFile(format!("Failed to set permissions: {}", e))
            })?;
        }
...
mod tests {
    ...
    fn test_set_active_account_not_found() {
        ...
        match result {
            Err(crate::error::PlurcastError::Account(AccountError::NotFound(
                account,
                platform,
            ))) => {
    ...
    fn test_multiple_platforms() {
        ...
        manager
            .register_account("mastodon", "test-mastodon")
            .unwrap();
        ...
        manager
            .set_active_account("mastodon", "test-mastodon")
            .unwrap();
