...
impl SSBKeypair {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.curve != "ed25519" {
            return Err(PlatformError::Authentication(format!(
                "Invalid curve: expected 'ed25519', got '{}'",
                self.curve
            ))
            .into());
        }
        ...
        let public_b64 = self.public.strip_suffix(".ed25519").ok_or_else(|| {
            PlatformError::Authentication("Invalid public key format".to_string())
        })?;
        let private_b64 = self.private.strip_suffix(".ed25519").ok_or_else(|| {
        ...
        if public_b64 != id_b64 {
            return Err(
                PlatformError::Authentication("ID does not match public key".to_string()).into(),
            );
        }

        BASE64.decode(public_b64).map_err(|e| {
            PlatformError::Authentication(format!("Invalid base64 in public key: {}", e))
        })?;

        BASE64.decode(private_b64).map_err(|e| {
            PlatformError::Authentication(format!("Invalid base64 in private key: {}", e))
        })?;
    ...
    pub fn to_json(&self) -> Result<String> {
        serde_json::to_string_pretty(self).map_err(|e| {
            PlatformError::Authentication(format!("Failed to serialize keypair: {}", e)).into()
        })
    }

    /// Deserialize a keypair from JSON string
    pub fn from_json(json: &str) -> Result<Self> {
        let keypair: Self = serde_json::from_str(json).map_err(|e| {
            PlatformError::Authentication(format!("Failed to parse keypair JSON: {}", e))
        })?;
