...
impl SSBPlatform {
    ...
    pub fn new(config: &SSBConfig) -> Self {
        ...
        tracing::info!("Creating SSB platform with feed database at: {}", feed_path.display());
        tracing::debug!("SSB configuration: enabled={}, pubs={}", config.enabled, config.pubs.len());
        
        let pub_connections: Vec<PubConnection> = config.pubs.iter()
            .filter_map(|addr_str| {
                match super::replication::PubAddress::parse(addr_str) {
                    Ok(addr) => {
        ...
        if config.pubs.is_empty() {
            ...
            tracing::info!("Configured {} pub server(s) for replication", pub_connections.len());
        }
...
impl Platform for SSBPlatform {
    ...
    async fn post(&self, post: &crate::Post) -> Result<String> {
        ...
        tracing::debug!("Message signed successfully, total size: {} bytes", message_size);
