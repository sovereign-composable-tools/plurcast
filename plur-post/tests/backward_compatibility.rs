...
fn test_output_format_unchanged_for_valid_content() {
    ...
    assert!(stdout.starts_with("draft:"), "Text format should start with 'draft:'");
    assert!(stdout.contains("-"), "Text format should contain UUID with dashes");
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("\"status\""), "JSON should contain status field");
    assert!(stdout.contains("\"draft\""), "JSON should contain draft status");
    assert!(stdout.contains("\"post_id\""), "JSON should contain post_id field");
}
