...
fn test_help_comprehensive_and_parseable() {
    ...
    assert!(stdout.lines().count() > 10, "Help should have multiple lines");
}
...
fn test_json_format_produces_valid_json() {
    ...
    assert!(json.get("post_id").is_some(), "JSON should have post_id field");
...
fn test_non_tty_plain_output() {
    ...
    assert!(!stdout.contains("\x1b["), "Output should not contain ANSI color codes when piped");
}
...
fn test_diagnostic_messages_to_stderr() {
    ...
    let lines: Vec<&str> = stdout.lines().collect();
    assert_eq!(lines.len(), 1, "Stdout should only contain post ID even in verbose mode");
}
...
fn test_exit_codes_documented_in_help() {
    ...
    assert!(stdout.contains("1") && (stdout.contains("Posting") || stdout.contains("posting") || stdout.contains("failed")));
    assert!(stdout.contains("2") && (stdout.contains("Authentication") || stdout.contains("authentication") || stdout.contains("auth")));
    assert!(stdout.contains("3") && (stdout.contains("Invalid") || stdout.contains("invalid") || stdout.contains("input")));
}
...
fn test_help_shows_usage_examples() {
    ...
    assert!(stdout.contains("USAGE EXAMPLES") || stdout.contains("Examples:") || stdout.contains("EXAMPLES"));
...
fn test_text_format_machine_readable() {
    ...
    assert_eq!(parts.len(), 2, "Text output should be in platform:id format");
}
