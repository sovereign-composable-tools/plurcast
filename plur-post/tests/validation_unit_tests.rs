...
fn test_error_message_includes_size_information() {
    ...
    assert!(
        stderr.contains("Content too large"),
        "Error message should contain 'Content too large'"
    );
    assert!(
        stderr.contains("exceeds 100000 bytes") || stderr.contains("100000 bytes"),
        "Error message should contain maximum size (100000 bytes)"
    );
    assert!(
        stderr.contains("maximum: 100000 bytes"),
...
fn test_error_message_no_content_samples() {
    ...
    assert!(
        !stderr.contains("SENSITIVE_DATA"),
...
fn test_argument_validation_under_limit() {
    ...
    cmd.arg(&content).arg("--draft").assert().success().code(0);
}
...
fn test_unicode_content_size_validation() {
    ...
    cmd.write_stdin(content.as_bytes())
        ...
        .code(3)
        .stderr(
            predicate::str::contains("Content too large")
                .or(predicate::str::contains("Failed to read from stdin")),
        );
}
...
fn test_exit_code_consistency_for_validation_errors() {
    ...
    let test_cases = vec![("", "empty content"), ("   \n\t  ", "whitespace only")];
