...
async fn run_command(command: Commands) -> Result<()> {
    match command {
        ...
        Commands::Delete { platform, force } => {
            delete_credentials(&platform, force).await
        }
        ...
        Commands::Migrate => {
            migrate_credentials().await
        }
        Commands::Audit => {
            audit_credentials().await
        }
    }
}
...
async fn set_credentials(platform: &str) -> Result<()> {
    ...
    println!("✓ Stored {} credentials securely using {} backend", 
        platform, 
...
async fn list_credentials() -> Result<()> {
    ...
    let platforms = vec![
        ...
        ("mastodon", "plurcast.mastodon", "access_token", "Access Token"),
        ("bluesky", "plurcast.bluesky", "app_password", "App Password"),
    ];
    ...
    for (platform_name, service, key, credential_type) in platforms {
        if manager.exists(service, key)? {
            ...
            println!("  ✓ {}: {} (stored in {})", platform_name, credential_type, backend);
            found_any = true;
        }
    }
...
async fn audit_credentials() -> Result<()> {
    ...
    if let Some(cred_config) = &config.credentials {
        ...
        if manager.is_insecure() {
            ...
            println!("✓ Using secure credential storage: {}", 
                manager.primary_backend().unwrap_or("unknown"));
            println!();
        }
    } else {
